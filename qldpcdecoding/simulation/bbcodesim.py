...
def circuit_level_simulation(code, error_rate, decoders,
    ...
    ## save the chk and obs matrices for later use
    ...
    np.save(f"test/chk_{n}.npy", chk)
    np.save(f"test/obs_{n}.npy", obs)
    np.save(f"test/priors_{n}.npy", priors)
    anchors = []
    j = 0
    for i in range(num_col):
        nnz_col = np.nonzero(chk[:, i])[0]
    ...
    for i in range(num_win):
        ...
        top_left += F
    if plot:
        plt.savefig(f"circuit_level_simulation_{n}_{num_win}.png")
    ...
    for decoder in decoders:
        ...
        num_sydrome = np.sum(det_data, axis=1)
        ## counts the distribution
        num_sydrome_dist = np.bincount(num_sydrome)
        print(f"Sydrome distribution: {num_sydrome_dist}\n sydrome range: {num_sydrome.min()}-{num_sydrome.max()}")
        assert num_sydrome.shape == (num_trials,)
        print
        start_time = time.perf_counter()
        top_left = 0
        decodingtime_list = []
        ...
        for i in range(num_win):
            ...
            for j in range(num_trials):
                ...
                if decoder.name == "BP":
                    iter_num = decoder.decoder.iter
                    decodingtime_list.append(iter_num*1e-6/250)  # 250MHz is the clock frequency of U50
                else:
