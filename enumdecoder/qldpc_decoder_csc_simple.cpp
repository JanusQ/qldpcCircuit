...
private:
    ...
    // Precompute likelihood table using CSC format (CPU version)
    void precompute_likelihoods_csc() {
        ...
        for (const Vec& e : errors) {
            // Compute syndrome using CSC matrix-vector multiplication
            Vec s_full = Dfull_csc.multiply_mod2(e);
            Vec s = Vec(s_full.begin(), s_full.begin() + D_csc.rows);
            Vec sl = Vec(s_full.begin() + D_csc.rows, s_full.end());
